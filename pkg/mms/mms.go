...
func GetMMSData(countries map[string]string) []MMSData {
	var unpackJson []MMSData
	var result []MMSData
	...
	err = json.Unmarshal(body, &unpackJson)
	if err != nil {
		return nil
	}
	for _, val := range unpackJson {
		res := checkCountry(val.Country, countries)
		if res {
...
func GetResultMMS() [][]MMSData {
	...
	resSMS := GetMMSData(countries)
	sortByName := make([]MMSData, len(resSMS))
	sortByProvider := make([]MMSData, len(resSMS))
