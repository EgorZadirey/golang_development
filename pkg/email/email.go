...
func GetEmailData() []EmailData {
	...
	content, err := ioutil.ReadFile("email.data")
	if err != nil {
		panic(err)
	}
	splitContent := strings.Split(string(content), "\n")
	for _, i := range splitContent {
		if len(strings.Split(i, ";")) != 3 {
			continue
		} else {
			if !checkProvider(strings.Split(i, ";")[1]) {
				continue
			} else {
				res := checkCountry(strings.Split(i, ";")[0], countries)
				if res {
					val, _ := strconv.Atoi(strings.Split(i, ";")[2])
					emailData := EmailData{
						Country:      strings.Split(i, ";")[0],
						Provider:     strings.Split(i, ";")[1],
						DeliveryTime: val,
					}
...
func GetResultEmail() map[string][][]EmailData {
	result := make(map[string][][]EmailData)
	resEmail := GetEmailData()
	fastDelivery := make([]EmailData, len(resEmail))
	lowDelivery := make([]EmailData, len(resEmail))
	for _, val := range resEmail {
		result[val.Country] = [][]EmailData{}
	}
	copy(fastDelivery, resEmail)
	copy(lowDelivery, resEmail)

	sort.Slice(fastDelivery, func(p, q int) bool {
		return fastDelivery[p].DeliveryTime < fastDelivery[q].DeliveryTime
	})
	sort.Slice(lowDelivery, func(p, q int) bool {
		return lowDelivery[p].DeliveryTime > lowDelivery[q].DeliveryTime
	})
	//fmt.Println("FAST", fastDelivery)
	//fmt.Println("LAW", lowDelivery)
	//result = append(result, fastDelivery)
	//result = append(result, lowDelivery)
	return result
}
