...
func GetVoiceCallData() []VoiceCallData {
	...
	content, err := ioutil.ReadFile("voice.data")
	if err != nil {
		panic(err)
	}
	splitContent := strings.Split(string(content), "\n")
	for _, i := range splitContent {
		if len(strings.Split(i, ";")) != 8 {
			continue
		} else {
			if !checkProvider(strings.Split(i, ";")[3]) {
				...
				if res {
					val1, _ := strconv.ParseFloat(strings.Split(i, ";")[4], 32)
					valFloat32 := float32(val1)
					val2, _ := strconv.Atoi(strings.Split(i, ";")[5])
					val3, _ := strconv.Atoi(strings.Split(i, ";")[6])
					val4, _ := strconv.Atoi(strings.Split(i, ";")[7])
					voiceData := VoiceCallData{
						...
						ConnectionStability: valFloat32,
						TTFB:                val2,
						VoicePurity:         val3,
						MedianOfCallsTime:   val4,
					}
					result = append(result, voiceData)
				}
			}
		}
	}
